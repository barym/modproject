...
static u16 FontFunc_Battle(struct TextPrinter *);
static u16 FontFunc_Level(struct TextPrinter *);
static u16 FontFunc_Hp(struct TextPrinter *);
...
static void DecompressGlyph_Battle(u16, bool32);
static void DecompressGlyph_Level(u16, bool32);
static void DecompressGlyph_Hp(u16, bool32);
...
static u32 GetGlyphWidth_Battle(u16, bool32);
static u32 GetGlyphWidth_Level(u16, bool32);
static u32 GetGlyphWidth_Hp(u16, bool32);
...
static const struct GlyphWidthFunc sGlyphWidthFuncs[] =
{
    ...
    { FONT_BATTLE,       GetGlyphWidth_Battle },
    { FONT_LEVEL,        GetGlyphWidth_Level },
    { FONT_HP,           GetGlyphWidth_Hp },
};
...
static u16 FontFunc_Battle(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_BATTLE;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}

static u16 FontFunc_Level(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_LEVEL;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}

static u16 FontFunc_Hp(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_HP;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case FONT_BRAILLE:
            break;
        case FONT_BATTLE:
            DecompressGlyph_Battle(currChar, textPrinter->japanese);
            break;
        case FONT_LEVEL:
            DecompressGlyph_Level(currChar, textPrinter->japanese);
            break;
        case FONT_HP:
            DecompressGlyph_Hp(currChar, textPrinter->japanese);
            break;
        }
...
static void DecompressGlyph_Battle(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontBattleLatinGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontBattleLatinGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Battle(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontBattleLatinGlyphWidths[glyphId];
}

static void DecompressGlyph_Level(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontLevelLatinGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontLevelLatinGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Level(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontLevelLatinGlyphWidths[glyphId];
}

static void DecompressGlyph_Hp(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;
    ...
    {
        glyphs = gFontHpLatinGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontHpLatinGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Hp(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontHpLatinGlyphWidths[glyphId];
}