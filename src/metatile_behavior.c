...
static const u8 sTileBitAttributes[NUM_METATILE_BEHAVIORS] =
{
    ...
    [MB_ASH_LONG_GRASS]                  = TILE_FLAG_UNUSED | TILE_FLAG_HAS_ENCOUNTERS,
};
...
bool8 MetatileBehavior_IsPokeGrass(u8 metatileBehavior)
{
    if (metatileBehavior == MB_TALL_GRASS
     ...
     || metatileBehavior == MB_ASH_LONG_GRASS)
        return TRUE;
    else
        return FALSE;
}
...
bool8 MetatileBehavior_IsAshLongGrass(u8 metatileBehavior)
{
    if (metatileBehavior == MB_ASH_LONG_GRASS)
        return TRUE;
    else
        return FALSE;
}
